...
export class BranchCreatorComponent implements AfterViewInit {
  ...
  @ViewChild('SettingComponent', {static: false}) settingComponent: SettingComponent;
  ...
  addBranch() {
    ...
    const branch = {coordinates: this.map.getCenter(), name: name.length > 2 ? [name[1], name[name.length - 1]].join(',') : [name[0], name[1]].join(',')};
      ...
      .end((error, response) => {
        ...
        if (response) {
          if (response.status === 200) {
            ...
            timer(2000)
              .subscribe(() => this.formattedAddress = null);
  ...
  confirmBranch() {
    ...
    if (this.settingComponent && this.settingComponent.isChecked) {
      const data = { lastSet: Date.now(), isRemember: this.settingComponent.isChecked, index: this.branch.index };
