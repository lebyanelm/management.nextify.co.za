            ...
            if (response) {
              if (response.status === 200) {
                for (const orderId of orderIds) {
                  for (
                    let index = 0;
                    index < this.sockets.data.orders[this.branch.id].length;
                    index++
                  ) {
                    if (
                      this.sockets.data.orders[this.branch.id][index].id ===
                      orderId
                    ) {
                      this.sockets.data.orders[this.branch.id][index].status =
                        status;
                      this.sockets.data.orders[this.branch.id][
                        index
                      ].driverName = response.body.driverName;
                      if (status === 4) {
                        ...
                        this.sockets.data.completedOrders[this.branch.id].push(
                          this.sockets.data.orders[
                            this.sockets.data.orders[this.branch.id][index]
                          ]
                        );
                        this.sockets.data.orders[this.branch.id].splice(
                          index,
                          1
                        );
                      }
              ...
              this.toast.show(
                response.body.reason || "ERROR: NO INTERNET CONNECTION."
              );
              reject();
            }
          });
    ...
    return new Promise((resolve, reject) => {
      ...
      superagent
        ...
        .end((_, response) => {
          if (response) {
            if (response.status === 200) {
              const orderIndex = this.sockets.data.orders[
                this.branch.id
              ].findIndex((o) => o.id === orderId);
